const menuList = [
    {
        country: '日本',
        record: [
            {
                date: '2014.08',
                title: '日本東京自由行(東京)',
                url: '/japan/201408-tokyo'
            },
            {
                date: '2016.08',
                title: '日本關西地方自由行(大阪、京都、奈良)',
                url: '/japan/201608-osaka'
            },
            {
                date: '2017.04',
                title: '日本關東地方自由行(東京、鎌倉、日光、橫濱)',
                url: '/japan/201704-tokyo'
            },
            {
                date: '2017.09',
                title: '日本中國地方自由行(中國地方、山陰山陽地區、出雲、松江、倉敷、岡山、姬路)',
                url: '/japan/201709-okayama'
            },
            {
                date: '2018.01',
                title: '日本東北地方自由行(山形、立石寺、銀山溫泉、藏王、會津若松、大內宿)',
                url: '/japan/201801-yamagata'
            },
            {
                date: '2018.04',
                title: '日本關西地方自由行(北九州、博多、廣島、京都、和歌山)',
                url: '/japan/201804-okayama'
            },
            {
                date: '2018.10',
                title: '日本關東地方自由行(東京、橫濱、富士山河口湖、輕井澤)',
                url: '/japan/201810-tokyo'
            },
            {
                date: '2019.09',
                title: '日本關東地方自由行(東京、長野、松本、新潟、越後、甲信越地方)',
                url: '/japan/201909-nagano'
            },
            {
                date: '2019.12',
                title: '日本關西地方自由行(大阪、京都、滋賀)',
                url: '/japan/201912-kyoto'
            },
            {
                date: '2022.11',
                title: '日本關東地方自由行(長野、松本、東京、關東地方)',
                url: '/japan/202211-japan'
            },
            {
                date: '2022.12',
                title: '日本東京自由行(東京)',
                url: '/japan/202212-tokyo'
            },
            {
                date: '2023.04',
                title: '日本關西地方自由行(京都、大阪、和歌山、那智熊野三山)',
                url: '/japan/202304-kyoto'
            },
            {
                date: '2023.06',
                title: '日本沖繩地方自由行(那霸、名護)',
                url: '/japan/202306-okinawa'
            },
            {
                date: '2023.09',
                title: '日本北海道自由行(花卷、札幌、函館)',
                url: '/japan/202309-hokkaido'
            },
            {
                date: '2024.02',
                title: '日本九州地方自由行(福岡、大分、鹿兒島、宮崎、熊本)',
                url: '/japan/202402-kyushu'
            },
            {
                date: '2024.11',
                title: '日本員工旅遊(沖繩)',
                url: '/#'
            },
            {
                date: '2025.01',
                title: '日本關西地方自由行(京阪跨年)',
                url: '/japan/202501-japan'
            },
            {
                date: '2025.02',
                title: '日本東北地方自由行(名古屋、秋田、岩手、青森)',
                url: '/japan/202502-japan'
            },
            {
                date: '2025.05',
                title: '日本中部地方自由行(立山黑部、金澤、白川鄉)',
                url: '/japan/202505-japan'
            },
            {
                date: '2025.09',
                title: '日本員工旅遊(九州)',
                url: '/#'
            },
            {
                date: '2026.06',
                title: '日本東京自由行(東京)',
                url: '/#'
            },
            {
                date: '2026.11',
                title: '日本東京自由行(東京、箱根、輕井澤)',
                url: '/japan/202611-japan'
            },
            {
                date: '',
                title: '日本四國地方自由行(高知、松山、德島)',
                url: ''
            },
            {
                date: '',
                title: '日本北海道道北地方自由行(札幌、小樽、旭川、稚內、網走)',
                url: ''
            },
            {
                date: '',
                title: '日本北海道道南地方自由行(函館、帶廣、根室、釧路)',
                url: ''
            },
        ]
    },
    {
        country: '歐洲',
        record: [
            {
                date: '2024.05',
                title: '瑞法自由行(瑞士、法國霞慕尼)',
                url: '/europe/202405-swiss'
            },
            {
                date: '2026.02',
                title: '義大利自由行(米蘭、威尼斯、佛羅倫斯、羅馬)',
                url: '/europe/202602-italy'
            },
            {
                date: '',
                title: '北歐自由行(挪威)',
                url: ''
            },
            {
                date: '',
                title: '西歐自由行(英法)',
                url: ''
            },
            {
                date: '',
                title: '西歐自由行(西班牙、葡萄牙)',
                url: ''
            },
            {
                date: '',
                title: '東歐自由行(捷克、波蘭、保佳利亞)',
                url: ''
            },
            {
                date: '',
                title: '東歐自由行(克羅埃西亞)',
                url: ''
            },
            {
                date: '',
                title: '冰島自由行(冰島環島)',
                url: ''
            },
            {
                date: '',
                title: '德國自由行(柏林、慕尼黑、法蘭克福；國王湖、新天鵝堡)',
                url: ''
            },
        ]
    },
    {
        country: '美加',
        record: [
            {
                date: '',
                title: '美東自由行(紐約、波士頓)',
                url: ''
            },
            {
                date: '',
                title: '美西自由行(洛杉磯、舊金山)',
                url: ''
            },
        ]
    },
    {
        country: '南韓',
        record: [
            {
                date: '2023.11',
                title: '韓國員工旅遊(釜山)',
                url: '/#'
            },
        ]
    }
]
export default menuList;