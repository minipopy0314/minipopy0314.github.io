const Record = {
    title: '2024 瑞法自由行',
    date: '2024/05/24 ~ 06/09',
    location: '瑞士、法國霞慕尼',
    flight: [
        {
            company: 'Emirates',
            airplane: '',
            takeoff: '臺北市 TPE 05月24日-23:35(GMT+8)',
            land: '杜拜 DXB 05月25日-05:05(GMT+4)',
            airport: '(臺北)桃園機場第1航廈',
            amount: 0,
            luggage: '25公斤(*3)'
        },
        {
            company: 'Emirates',
            airplane: '',
            takeoff: '杜拜 DXB 05月25日-08:40(GMT+4)',
            land: '蘇黎世 ZRH 05月25日-13:20(GMT+2)',
            airport: '(阿聯)杜拜機場',
            amount: 0,
            luggage: '25公斤(*3)'
        },
        {
            company: 'Emirates',
            airplane: '',
            takeoff: '蘇黎世 ZRH 06月09日-15:25(GMT+2)',
            land: '杜拜 DXB 06月09日-23:45(GMT+4)',
            airport: '(瑞士)蘇黎世機場第1航廈',
            amount: 0,
            luggage: '30公斤(*3)'
        },
        {
            company: 'Emirates',
            airplane: '',
            takeoff: '杜拜 DXB 06月10日-03:40(GMT+4)',
            land: '臺北市 TPE 06月10日-16:15(GMT+8)',
            airport: '(阿聯)杜拜機場',
            amount: 108381,
            luggage: '30公斤(*3)'
        },
    ],
    lodging: [
        {
            name: 'The Bed + Breakfast',
            address: '',
            tel: '',
            google_map: 'https://www.thebandb.ch/',
            room: [
                {
                    order_no: '',
                    day: '3',
                    room_type: '三人房共用衛浴',
                    user: '3'
                }
            ],
            amount: '580.50 瑞士法郎'
        },
        {
            name: 'Gasthof Schoenegg B&B',
            address: '',
            tel: '',
            google_map: 'https://www.gasthof-schoenegg.ch/?lang=en',
            room: [
                {
                    order_no: '4110128744',
                    day: '2',
                    room_type: '公寓套房',
                    user: '3'
                }
            ],
            amount: '535.50 瑞士法郎'
        },
        {
            name: 'Daniela house',
            address: '',
            tel: '',
            google_map: '',
            room: [
                {
                    order_no: '',
                    day: '3',
                    room_type: '公寓套房',
                    user: '3'
                }
            ],
            amount: '780.80 瑞士法郎'
        },
        {
            name: 'Grepon 102 house',
            address: '',
            tel: '',
            google_map: 'https://www.happy.rentals/view/5408-chamonix-sud-grepon-102',
            room: [
                {
                    order_no: '',
                    day: '2',
                    room_type: '公寓套房',
                    user: '3'
                }
            ],
            amount: '208 瑞士法郎'
        },
        {
            name: 'Haus Andorra',
            address: '',
            tel: '+41279676681',
            google_map: 'https://www.apartment-rental-zermatt.com/',
            room: [
                {
                    order_no: '',
                    day: '3',
                    room_type: '公寓套房',
                    user: '3'
                }
            ],
            amount: '814.30 瑞士法郎'
        },
        {
            name: 'Hotel Saint Georges',
            address: '',
            tel: '',
            google_map: 'https://www.hotel-st-georges.ch/en/',
            room: [
                {
                    order_no: '1044390556',
                    day: '1',
                    room_type: '三人房共用衛浴',
                    user: '3'
                }
            ],
            amount: '167.08 瑞士法郎'
        },
    ],
    cost: {

    },
    journey: [
        {
            date: '5/24',
            title: '啟程',
            log: [
                {
                    time: '23:35(GMT+8)',
                    title: '出發',
                    detail: 'Emirates 航班 台北飛往杜拜 5/25 05:05(GMT+4) 抵達',
                    url: ''
                },
            ]
        },
        {
            date: '5/25',
            title: '蘇黎世(Zurich)、琉森(Luzern)',
            log: [
                {
                    time: '08:40(GMT+4)',
                    title: '杜拜轉機',
                    detail: 'Emirates 航班 杜拜飛往蘇黎世 5/25 13:20(GMT+2) 抵達',
                    url: ''
                },
                {
                    time: '13:20',
                    title: '抵達蘇黎世',
                    detail: '購買網卡和兌換旅遊通票並準備搭車至蘇黎世車站轉乘至琉森',
                    url: ''
                },
                {
                    time: '',
                    title: '琉森車站',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: 'Check in',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '琉森市區晃晃',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '宿',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '5/26',
            title: '琉森、瑞吉山(Rigi)',
            log: [
                {
                    time: '',
                    title: '瑞吉山',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '琉森晃晃',
                    detail: '機動行程如果前天琉森有景點沒去可以今天補',
                    url: ''
                },
            ]
        },
        {
            date: '5/27',
            title: '皮拉圖斯山(Pilatus) 金色環遊路線',
            log: [
                {
                    time: '',
                    title: '皮拉圖斯山',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '5/28',
            title: '因特拉肯(Interlaken)、布里恩茲湖(Brienz) 遊湖、哈德昆觀景台(Harder Kulm)',
            log: [
                {
                    time: '',
                    title: '琉森',
                    detail: '搭乘黃金列車前往因特拉肯',
                    url: ''
                },
                {
                    time: '',
                    title: '因特拉肯',
                    detail: '先搭公車去飯店寄放行李',
                    url: ''
                },
                {
                    time: '',
                    title: '布里恩茲湖',
                    detail: '搭船遊湖',
                    url: ''
                },
                {
                    time: '',
                    title: '哈德昆觀景台',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: 'Check in',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '5/29',
            title: '藍湖(Blausee)、厄希嫩湖(Oeschinensee)',
            log: [
                {
                    time: '',
                    title: '藍湖(Blausee)',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '厄希嫩湖(Oeschinensee)',
                    detail: '',
                    url: ''
                }
            ]
        },
        {
            date: '5/30',
            title: '格林德瓦(Grindelwald)、少女峰 First',
            log: [
                {
                    time: '',
                    title: 'Check out',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '因特拉肯',
                    detail: '前往格林德瓦',
                    url: ''
                },
                {
                    time: '',
                    title: '格林德瓦',
                    detail: '可能要找地方放行李',
                    url: ''
                },
                {
                    time: '',
                    title: '少女峰 First',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: 'Check in',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '5/31',
            title: '少女峰、少女峰車站、小夏克戴',
            log: [
                {
                    time: '',
                    title: '格林德瓦',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '小夏克戴',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '少女峰車站',
                    detail: '',
                    url: ''
                }
            ]
        },
        {
            date: '6/1',
            title: '雪朗峰、米倫小鎮',
            log: [
                {
                    time: '',
                    title: '米倫小鎮',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '雪朗峰',
                    detail: '',
                    url: ''
                }
            ]
        },
        {
            date: '6/2',
            title: '蒙特魯(Montreux)、西傭城堡、黃金列車 GoldenPass Belle Epoque、日內瓦(Geneva)',
            log: [
                {
                    time: '',
                    title: 'Check out',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '因特拉肯',
                    detail: '搭乘黃金列車 GoldenPass Belle Epoque前往蒙特魯',
                    url: ''
                },
                {
                    time: '',
                    title: '蒙特魯',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '西傭城堡',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '日內瓦',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: 'Check in',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '6/3',
            title: '法國安錫(Annecy)、霞慕尼(Chamonix)',
            log: [
                {
                    time: '',
                    title: 'Check out',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '安錫',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '芒通聖伯納城堡',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '安錫老城區、安錫城堡',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '霞慕尼',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: 'Check in',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '6/4',
            title: '霞慕尼(Chamonix)、白朗峰',
            log: [
                {
                    time: '',
                    title: '霞慕尼',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '白朗峰',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '6/5',
            title: '霞慕尼(Chamonix)、策馬特(Zermatt)',
            log: [
                {
                    time: '',
                    title: 'Check out',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '霞慕尼',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '策馬特',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: 'Check in',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '6/6',
            title: '馬特洪峰 Gornergrat、冰川天堂',
            log: [
                {
                    time: '',
                    title: 'Gornergrat',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '冰川天堂',
                    detail: '',
                    url: ''
                }
            ]
        },
        {
            date: '6/7',
            title: '馬特洪峰五湖健行',
            log: [
                {
                    time: '',
                    title: '五湖健行',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '6/8',
            title: '伯恩(Bern)、蘇黎世(Zurich)',
            log: [
                {
                    time: '',
                    title: 'Check out',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '伯恩',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: '蘇黎世',
                    detail: '',
                    url: ''
                },
                {
                    time: '',
                    title: 'Check in',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '6/9',
            title: '蘇黎世(Zurich)',
            log: [
                {
                    time: '7:00',
                    title: '蘇黎世市區閒晃',
                    detail: '',
                    url: ''
                },
                {
                    time: '10:00',
                    title: 'Check out',
                    detail: '',
                    url: ''
                },
                {
                    time: '11:00',
                    title: '前往蘇黎世機場',
                    detail: '',
                    url: ''
                },
                {
                    time: '15:25(GMT+2)',
                    title: '賦歸',
                    detail: 'Emirates 航班 蘇黎世飛往杜拜 6/9 23:45(GMT+4) 抵達',
                    url: ''
                },
            ]
        },
        {
            date: '6/10',
            title: '回到溫暖的家',
            log: [
                {
                    time: '03:40(GMT+4)',
                    title: '杜拜轉機',
                    detail: 'Emirates 航班 杜拜飛往台北 6/10 16:15(GMT+8) 抵達',
                    url: ''
                },
                {
                    time: '16:15(GMT+8)',
                    title: '賦歸',
                    detail: '抵達台北',
                    url: ''
                }
            ]
        },
    ]
}
export default Record