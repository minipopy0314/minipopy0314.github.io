const Record = {
    title: '2025 日本東北地方自由行',
    date: '2025/01/30 ~ 02/06',
    location: '名古屋、秋田、岩手、青森',
    flight: [
        {
            company: '樂桃航空 Peach Air Inc.',
            airplane: 'MM722',
            takeoff: '臺北市 TPE 01月30日-02:45(GMT+8)',
            land: '名古屋 HNA 01月30日-06:20(GMT+9)',
            airport: '(臺北)桃園機場第一航廈',
            amount: 0,
            luggage: '單程20公斤(*3)'
        },
        {
            company: '樂桃航空 Peach Air Inc.',
            airplane: 'MM627',
            takeoff: '東京 HNA 02月06日-22:05(GMT+9)',
            land: '臺北市 TPE 02月07日-01:25(GMT+8)',
            airport: '(日本)成田國際機場第一航廈',
            amount: 11620,
            luggage: '單程20公斤(*3)'
        }
    ],
    lodging: [
        {
            name: 'Comfort INN 名古屋榮站前',
            address: '3 Chome-16-30 Nishiki, Naka Ward, Nagoya, Aichi 460-0003日本',
            tel: '+81 52-951-1411',
            google_map: '',
            room: [
                {
                    order_no: '',
                    day: '2025年1月30日(星期四) 開始 1 宿',
                    room_type: '雙床+沙發床',
                    user: '3'
                }
            ],
            amount: '13,300 日圓'
        },
        {
            name: '仙台蒙特利飯店',
            address: '4 Chome-1-8 Central, Aoba Ward, Sendai, Miyagi 980-0021日本',
            tel: '+81 22-265-7110',
            google_map: '',
            room: [
                {
                    order_no: '',
                    day: '2025年1月31日(星期五) 開始 1 宿',
                    room_type: '禁菸房',
                    user: '3'
                },
            ],
            amount: '21,706 日圓'
        },
        {
            name: '乳頭溫泉鄉 妙乃湯',
            address: 'Komagatake-2-1 Tazawako Obonai, Semboku, Akita 014-1201日本',
            tel: '+81 187-46-2740',
            google_map: '',
            room: [
                {
                    order_no: '',
                    day: '2025年2月1日(星期六) 開始 1 宿',
                    room_type: '和室',
                    user: '3'
                }
            ],
            amount: '72,000 日圓'
        },
        {
            name: 'MYSTAYS 青森駅',
            address: '1 Chome-8-6 Shinmachi, Aomori, 030-0801日本',
            tel: '+81 17-721-5656',
            google_map: '',
            room: [
                {
                    order_no: '',
                    day: '2025年2月2日(星期日) 開始 2 宿',
                    room_type: '三床房',
                    user: '3'
                }
            ],
            amount: '40,967 日圓'
        },
        {
            name: 'APA仙台駅北',
            address: '201-1 Nakakecho, Miyagino Ward, Sendai, Miyagi 983-0864',
            tel: '+81 22-742-3630',
            google_map: '',
            room: [
                {
                    order_no: '',
                    day: '2025年2月4日(星期二) 開始 1 宿',
                    room_type: '雙人房',
                    user: '2'
                },
                {
                    order_no: '',
                    day: '2025年2月4日(星期二) 開始 1 宿',
                    room_type: '雙人房',
                    user: '2'
                }
            ],
            amount: '14,567 日圓'
        },
        {
            name: '秋保溫泉 瑞鳳',
            address: '982-0241 宮城県仙台市太白区秋保町湯元字除26-1',
            tel: '0570-550-397',
            google_map: '',
            room: [
                {
                    order_no: '',
                    day: '2025年2月5日(星期三) 開始宿',
                    room_type: '和室',
                    user: '3'
                }
            ],
            amount: '44,329 日圓'
        },
    ],
    cost: {
        list: [
            
        ]
    },
    journey: [
        {
            date: '1/30',
            title: '名古屋',
            pay: [
            ],
            log: [
                {
                    time:'',
                    title: '出發',
                    detail: '去程 MM722 06:20 抵達 名古屋中部國際機場',
                    url: ''
                },
                {
                    time:'',
                    title: '名古屋中部國際機場',
                    detail: '搭乘鐵路至名古屋中央站',
                    url: ''
                },
                {
                    time:'',
                    title: 'Check in Comfort INN 名古屋榮站前',
                    detail: '寄放行李',
                    url: ''
                },
                {
                    time:'',
                    title: '早餐',
                    detail: '隨便吃',
                    url: ''
                },
                {
                    time:'',
                    title: '熱田神宮',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '中餐',
                    detail: '熱田蓬萊軒 神宮店 11:30 開始營業',
                    url: ''
                },
                {
                    time:'',
                    title: '名古屋城',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '大須觀音、大須商店街',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '萬松寺、三輪神社',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '晚餐',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '宿 Comfort INN 名古屋榮站前',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '1/31',
            title: '伊勢神宮',
            pay: [
            ],
            log: [
                {
                    time: '',
                    title: 'Check out Comfort INN 名古屋榮站前',
                    detail: '寄放行李',
                    url: ''
                },
                {
                    time:'',
                    title: '伊勢神宮',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '御蔭橫丁',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '名古屋站',
                    detail: '直達仙台（東京換乘）',
                    url: ''
                },
                {
                    time: '',
                    title: '宿 仙台蒙特利飯店',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '2/1',
            title: '田澤湖、乳頭溫泉鄉',
            pay: [
            ],
            log: [
                {
                    time:'',
                    title: 'Check out 仙台蒙特利飯店',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '田澤湖站',
                    detail: '寄放行李',
                    url: ''
                },
                {
                    time:'',
                    title: '田澤湖周遊巴士',
                    detail: '注意巴士班表',
                    url: ''
                },
                {
                    time:'',
                    title: '乳頭溫泉鄉',
                    detail: '注意巴士班表',
                    url: ''
                },
                {
                    time:'',
                    title: '晚餐',
                    detail: '旅館提供',
                    url: ''
                },
                {
                    time:'',
                    title: '宿 乳頭溫泉鄉 妙乃湯',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '2/2',
            title: '秋田內陸線',
            pay: [
            ],
            log: [
                {
                    time:'',
                    title: '早餐',
                    detail: '旅館提供',
                    url: ''
                },
                {
                    time:'',
                    title: 'Check out 乳頭溫泉鄉 妙乃湯',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '田澤湖站',
                    detail: '寄放行李',
                    url: ''
                },
                {
                    time:'',
                    title: '秋田內陸線',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '青森',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '宿 MYSTAYS 青森駅',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '2/3',
            title: '青森、十和田湖、八甲山纜車',
            pay: [
            ],
            log: [
                {
                    time:'',
                    title: '早餐',
                    detail: '旅館提供',
                    url: ''
                },
                {
                    time:'',
                    title: 'Check out MYSTAYS 青森駅',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '八甲山纜車',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '奧入瀨溪流',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '十和田湖',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '宿 APA仙台駅北',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '2/4',
            title: '青森、輕津鐵道、弘前城',
            pay: [
            ],
            log: [
                {
                    time:'',
                    title: '早餐',
                    detail: '旅館提供',
                    url: ''
                },
                {
                    time:'',
                    title: 'JR青森站',
                    detail: '搭乘第一班次愉快列車白神號',
                    url: ''
                },
                {
                    time:'',
                    title: 'JR五所川原站、津輕五所川原站',
                    detail: '轉乘暖爐列車',
                    url: ''
                },
                {
                    time:'',
                    title: '津輕金木站',
                    detail: '隨意逛逛後搭下一個班次折返',
                    url: ''
                },
                {
                    time:'',
                    title: 'JR弘前站',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '弘前城',
                    detail: '附近吃個蘋果派',
                    url: ''
                },
                {
                    time:'',
                    title: '宿 MYSTAYS 青森駅',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '2/5',
            title: '猊鼻溪、秋保溫泉',
            pay: [
            ],
            log: [
                {
                    time:'',
                    title: 'Check out APA仙台駅北',
                    detail: '寄放行李',
                    url: ''
                },
                {
                    time:'',
                    title: '猊鼻溪',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '秋保溫泉',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '宿 秋保溫泉 瑞鳳',
                    detail: '',
                    url: ''
                },
            ]
        },
        {
            date: '2/6',
            title: '秋保溫泉、東京',
            pay: [
            ],
            log: [
                {
                    time:'',
                    title: 'Check out 秋保溫泉 瑞鳳',
                    detail: '',
                    url: ''
                },
                {
                    time:'',
                    title: '秋保溫泉',
                    detail: '早上可以晃晃',
                    url: ''
                },
                {
                    time:'',
                    title: '仙台',
                    detail: '退房後回仙台轉新幹線回東京',
                    url: ''
                },
                {
                    time:'',
                    title: '東京',
                    detail: '',
                    url: ''
                },
                {
                    time:'14:40',
                    title: '成田空港',
                    detail: '20:00(GMT+9) 抵達',
                    url: ''
                },
                {
                    time:'22:00',
                    title: '賦歸',
                    detail: '回程 MM627 01:25(GMT+8) 抵達 桃園機場第一航廈',
                    url: ''
                },
            ]
        }

    ]
}
export default Record